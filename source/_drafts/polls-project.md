...
还有`res.render()`的第二个参数的类型，是一个对象包括很多键值对，还是一个键值对，值里是一个json？看到res.send方法中包含了对jsonp的支持代码，这个需要探索一下，这个第二参数怎么传，经过测试，第二参数随便怎么传都可以，但要保证第二个参数是一个对象，`res.render`中还对第二参数进行了self.local处理？这是什么待探索。对这个文件修改完了之后要对服务器进行重启，否则会找不到数据，对视图的改动可以不用重启
